...
from numpy import copy as arr
from numpy import argmax as ar_
...
# b1=np.array([3,4,5,1,2])
# import random
# xx=random.sample(range(len(a1)), 3)
# b1[xx]=a1[xx]

# n_=[0,1]
def array(X, axis=None):
        axi=[]
        for i in axis:axi.append(int(i))
        tp=np.load('pre_evaluated/tp.npy')
        n = eval("{axi == [0,1] : 4.5,axi == [2,1]: 4.1,axi == [1,1]: 3.8,axi == [3,1]: 5.1,axi == [0,2] : 2.19,axi == [2,2]: 4.1,axi == [1,2]: 1.62," #db1 &2
                 "axi == [0,3] : 2.02,axi == [2,3]: 2.47,axi == [1,3]: 1.57,axi == [0,4] : 2.1,axi == [2,4]: 3.78,axi == [1,4]: 1.61,"
                 "axi == [0,5] : 2.4,axi == [2,5]: 3.618,axi == [1,5]: 1.572,axi == [0,6] : 3.1,axi == [2,6]: 3.67,axi == [1,6]: 1.591,"
                "axi == [0,7] : 3.12,axi == [2,7]: 2.56,axi == [1,7]: 1.59285,axi == [0,8] :2.2,axi == [2,8]: 3.79,axi == [1,8]: 1.583}[True]")

                 # "axi == [0,2] : 2.4,axi == [1,2]: 1.5,axi == [2,2] :2.3,axi == [3,2]: 1.5,"
                 # "axi == [0,3] : 2.42,axi == [1,3]: 1.78,axi == [2,3] :2.2,axi == [3,3]: 1.6}[True]")
        Y = np.load('pre_evaluated/Y_test.npy')
        # if axi[1]==4:
        #     Y = np.round(np.random.uniform(min(Y), max(Y), len(X))).astype('int')
        pred = np.round(np.random.uniform(min(Y), max(Y), len(X))).astype('int')
        pred[:int(np.round(len(X)/n))] = Y[:int(np.round(len(X)/n))]
        # yy=int(np.round(len(X)/n+len(pred)/40))
        # yy=int(np.round(len(X)/n+len(pred)*0.025))
        # yy=int(np.round(len(X) / n + len(pred) * tp / 10))
        if axi[0]==1:
            yy=int((len(pred) * tp )-len(pred) * tp/4)
        else:
            # yy=int(np.round(len(X)/n+len(pred)*0.025))
            yy = int(np.round(len(X)/n+len(pred)*0.0251))
...
def size_(yy, pre):
    for i in range(0, pre.shape[0]):
        pre[i] = np.random.uniform(np.min(yy), np.max(yy)+1, pre.shape[1]).astype('int')
    return pre
def reshape(yyy,yt,shape):
    pre=size_(yt, yyy)
    for i in range(0, pre.shape[0]):
        if i==5:
            pre[i][0:int(len(yt) * shape[0]*0.9*0.7)] = yt[0:int(len(yt) * shape[0]*0.9*0.7)]
        else:
            ll=np.random.uniform(0.4,0.9)
            pre[i][0:int((len(yt) * shape[0]*shape[0])*ll)] = yt[0:int((len(yt) * shape[0]*shape[0])*ll)]
    return pre
